...
ChUdpClient.prototype.validateMessage = function (message) {
   ...
   if (message && message.hasOwnProperty ("config")) {
      ...
      if (config && config.hasOwnProperty ("http")) {
         ...
         if (http && http.hasOwnProperty ("ip") && http.hasOwnProperty ("port")
               && http.hasOwnProperty ("url") && http.ip && http.port
               && http.url && http.ip.length > 0 && http.url.length > 0) {
            valid = true;
         }
      }
   }
...
ChUdpClient.prototype.sendBroadcast = function (self) {
   ...
   self.udpHandle.bind (function () {
      ...
      self.udpHandle.send (message, 0, message.length,
            self.getBroadcastPort (), self.broadcastAddress, function (err,
                  bytes) {
               if (err)
                  throw err;
      ...
      self.udpHandle.on ("message", function (message, remote) {
         ...
         try {
            ...
            var machineDetails = self
                  .getMachineDetailsFromMessage (messageJson);
            if (machineDetails) {
